...
try:
    ...
    #  no scipy
    from pycnal.extern import KDTree

import pycnal
from pycnal.vgrid import *
from pycnal.extern import GreatCircle
...
class Gridgen(CGrid):
    ...
    def __init__(self, xbry, ybry, beta, shape, ul_idx=0, \
        ...
        self._libgridgen = np.ctypeslib.load_library('libgridgen', pycnal.__path__[0])
...
class edit_mask_mesh_ij(object):
    ...
    def __init__(self, grd, coast=None, **kwargs):
        ...
        if coast is None:
            ...
            map = pycnal.utility.get_grid_proj(grd, resolution='h', area_thresh=5)
            coast = pycnal.utility.get_coast_from_map(map)

        self.coast = coast

        ijcoast = pycnal.utility.ijcoast(self.coast, grd)
