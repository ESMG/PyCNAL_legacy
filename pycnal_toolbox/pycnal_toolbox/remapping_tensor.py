...
#            print datetime.datetime.now()
            ...
            # get source data
            src_11 = pycnal.utility.get_nc_var(varname[0], srcfile[nf])
            # get spval
            try:
            ...
            src_22 = pycnal.utility.get_nc_var(varname[1], srcfile[nf])
            src_22 = src_22[nt,jjrange[0]:jjrange[1],iirange[0]:iirange[1]]

            src_12 = pycnal.utility.get_nc_var(varname[2], srcfile[nf])
            src_12 = src_12[nt,jjrange[0]:jjrange[1],iirange[0]:iirange[1]]

            print("before", src_11[-1,30], src_12[-1,30], src_22[-1,30])
            if shapiro:
                src_11 = pycnal_toolbox.shapiro_filter.shapiro2(src_11,2)
                src_22 = pycnal_toolbox.shapiro_filter.shapiro2(src_22,2)
                src_12 = pycnal_toolbox.shapiro_filter.shapiro2(src_12,2)
            print("after", src_11[-1,30], src_12[-1,30], src_22[-1,30])

            # horizontal interpolation using scrip weights
            print('horizontal interpolation using scrip weights')
            dst_11 = pycnal.remapping.remap(src_11, wts_file, \
                                              spval=spval)
            dst_22 = pycnal.remapping.remap(src_22, wts_file, \
                                              spval=spval)
            dst_12 = pycnal.remapping.remap(src_12, wts_file, \
                                              spval=spval)
