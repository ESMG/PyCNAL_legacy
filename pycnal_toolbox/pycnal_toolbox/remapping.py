...
def remapping(varname, srcfile, wts_files, srcgrd, dstgrd, \
    ...
    if type(srcgrd).__name__ == 'ROMS_Grid':
        ...
        srcgrd = pycnal.grid.get_ROMS_grid(srcgrd)
    if type(dstgrd).__name__ == 'ROMS_Grid':
        dstgrd = dstgrd
    else:
        dstgrd = pycnal.grid.get_ROMS_grid(dstgrd)
    ...
    dst_zcoord = pycnal.vgrid.z_coordinate(dstgrd.vgrid.h, zlevel, nzlevel)
    ...
    for nf in range(nfile):
        ...
        if nctidx == 0:
            ...
            if os.path.exists(dstfile) is False:
                ...
                pycnal_toolbox.nc_create_roms_file(dstfile, dstgrd, ocean_time)
...
#                    raise Warning, 'Did not find a _FillValue attribute.' 
                ...
                if ndim == 3:
                    ...
                    src_varz = pycnal.remapping.roms2z( \
                    ...
                    src_varz = pycnal.remapping.flood(src_varz, srcgrdz, Cpos=Cpos, \
                ...
                if ndim == 3:
                    ...
                    dst_var = pycnal.remapping.z2roms(dst_varz, dstgrdz, dstgrd, \
            ...
            if rotate_uv is True:
                ...
                if ndim == 3:
                    src_vz = pycnal.remapping.roms2z( \
                    ...
                    src_vz = pycnal.remapping.flood(src_vz, srcgrdz, Cpos=Cpos_v, \
                                  irange=iirange, jrange=jjrange, \
                                  spval=spval, dmax=dmax, cdepth=cdepth, kk=kk)
                else:
                ...
                if ndim == 3:
                    # vertical interpolation from standard z level to sigma
                    print('vertical interpolation from standard z level to sigma')
                    dst_u = pycnal.remapping.z2roms(dst_uz, dstgrdz, dstgrd, \
                                 Cpos='rho', spval=spval, flood=False)
                    dst_v = pycnal.remapping.z2roms(dst_vz, dstgrdz, dstgrd, \
                                 Cpos='rho', spval=spval, flood=False)
                else:
                    dst_u = dst_uz
                    dst_v = dst_vz
                ...
                if rotate_part:
                    ...
                    src_angle = pycnal.remapping.remap(src_ang, wts_file)
