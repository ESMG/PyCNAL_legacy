...
def average(var, ncfiles, trange=None, avgfile=None, spval=1e37, timevar='ocean_time'):
    ...
    avg.ncfiles = pycnal.io.MFDataset(ncfiles)

    ocean_time = pycnal.utility.get_nc_var(timevar, avg.ncfiles)
    Nt = len(ocean_time[:])   
    ...
    for varname in var:
        name = varname
        vble = pycnal.utility.get_nc_var(varname,avg.ncfiles)
        vsh = vble.shape
        leng = len(vsh)
...
4D (time + 3 spacial dims)')
    ...
    if avgfile is not None:
        ...
        if type(avgfile).__name__ == 'str':
            ...
            nc.Author = 'pycnal_toolbox.average'
